...
test("every public namespace operation crosses the real Unix-socket seam", async () => {
  const server = await startHerdrTestServer((request, socket) => {
    ...
    if (request.method === "events.subscribe") {
      ...
      setTimeout(() => socket.write(`${JSON.stringify(eventResponse.result.event)}\n`), 10);
    }
    return response;
  });
  ...
  try {
    ...
    const notification = server.requests.find((request) => request.method === "notification.show");
    ...
    expect(
      server.requests.find(
        (request) => request.method === "workspace.close" && request.params.close_group === true,
      ),
    ).toBeDefined();
    expect(server.requests.find((request) => request.method === "pane.input.set")).toMatchObject({
